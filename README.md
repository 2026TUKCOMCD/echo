...
| Step 3 | 대화 시작 (Click) | 앱 메인 화면의 **[대화 시작하기]** 버튼 클릭 시 백그라운드에서 자동 실행됨<br/>1) Health Connect: 오늘 걸음 수 및 심박수 즉시 추출<br/>2) Fused Location Provider: 현재 위치 및 이동 경로 분석<br/>3) Stay Point Detection: 방문 장소(예: 공지천) 및 체류 시간 계산 |
| Step 4 | AI 맥락 통합 | 분석된 건강·위치 데이터를 프롬프트에 자동 주입하고, `[공지천 + 3,000보 + 심박수]` 맥락 데이터를 서버(GPT-4o-mini)로 즉시 전송 |
| Step 5 | 맞춤형 대화 생성 | AI가 분석된 맥락을 기반으로 TTS 음성 응답을 생성<br/>예시: **“영희님, 어서 오세요! 방금 공지천에서 3,000보나 걷고 오셨네요? 기분은 좀 어떠신가요?”** |
